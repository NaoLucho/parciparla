...
class BuildContentController extends Controller
{
    ...
    public function buildDefaultContentAction($selectposition, Request $request)
    {
        ...
        return $this->render('MNHNAdminBundle:BuildPage:buildcontents.html.twig', array(
            'page' => $page,
    ...
    public function buildContentAction($pageContent, $id = 0, Request $request)
    {
        ...
        switch ($pageContent->getContent()->getType()) {
            case "Image":
                return $this->render('MNHNAdminBundle:BuildContent:image.html.twig', array(
                    'pageContent' => $pageContent
                ));
                break;
            case "MainMenu":
                return $this->render('MNHNAdminBundle:BuildContent:menu.html.twig', array(
                    'pageContent' => $pageContent
                ));
                break;
            case "Content":
                return $this->render('MNHNAdminBundle:BuildContent:content.html.twig', array(
                    'pageContent' => $pageContent
                ));
                break;
            case "Menu":
            ...
            default: //Text
                return $this->render('MNHNAdminBundle:BuildContent:text.html.twig', array(
                    'pageContent' => $pageContent
                ));
        }
    }
    ...
    private function testMatchUrl($prefix, $url, $request)
    {
        try{
            ...
            if($route != null && is_array($route))
            {
                if(isset($route["_route"]) && $route["_route"] == "mnhn_buildpage")
                {
                    return null;
                }
                else{
